...
<Steps>
  ...
  <Step title="Create namespace and secrets">
    ...
    Create the image pull secret for `cr.takumo.io`. Issue a deploy token
    from the Takumo dashboard (Settings → Deploy Tokens) — the username is
    your workspace ID, the password is the token value. Tokens can be
    revoked at any time. Full reference:
    ...
    kubectl create secret docker-registry takumo-pull-secret \
      ...
      --docker-username=YOUR_TAKUMO_WORKSPACE_ID \
    ...
    **Air-gapped:** if your cluster has no egress to `cr.takumo.io`, mirror
    ...
    internal registry, override `image.repository` in your values file,
    and supply that registry's pull credentials in this secret instead.
  </Step>
